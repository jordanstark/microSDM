...
fpar_dat <- read.csv(paste(sp_path,"pls_points_FPAR.csv",sep=""),stringsAsFactors=F)
...
for(j in 1:length(ptdata$decimalLongitude)) {
  ...
  write.csv(dem_pt_list[[j]],paste(out_path,"microusa_20y_shade",j,".csv",sep=""))
}

dem_pt_df <- do.call(rbind,dem_pt_list)
write.csv(dem_pt_df,paste(out_path,"20y_Shade_microclimate_summary.csv",sep=""))
...
write.csv(mean_bioclim,paste(sp_path,"bioclim_20ymean_shademicroclimate.csv",sep=""))
