...
rbs_spatial <- read.csv(paste(sp_path,"all_rbs_points.csv",sep=""))
rbs_spatial$X <- NULL
rbs_spatial <- SpatialPointsDataFrame(coords=rbs_spatial[,c("decimalLongitude","decimalLatitude")],
                                      proj4string=crs("+proj=longlat"),data=rbs_spatial)
...
rbs_tr <- spTransform(rbs_spatial, crs(DEM)) #transform points to crs of DEM

rbs_tr$elev <- extract(DEM,rbs_tr)
rbs_tr$slope <- extract(slope,rbs_tr)
rbs_tr$aspect <- extract(aspect,rbs_tr)

ptdata <- data.frame(rbs_tr@data[,c("decimalLongitude","decimalLatitude","elev","slope","aspect","rbs_presence")])

# import FPAR data for shading
fpar_dat <- read.csv(paste(sp_path,"rbs_points_FPAR.csv",sep=""),stringsAsFactors=F)
fpar_ids <- unique(fpar_dat[,c("decimalLongitude","decimalLatitude","ID")])
fpar_ids$lon_lat <- paste(fpar_ids$decimalLongitude,fpar_ids$decimalLatitude,sep="_")

ptdata$fpar_id <- NA

for(i in 1:length(ptdata$decimalLatitude)) {
  pt_lon_lat <- paste(ptdata$decimalLongitude[i],ptdata$decimalLatitude[i],sep="_")
...
for(j in 1:length(ptdata$decimalLongitude)) {
  ...
  dat <- tryCatch(micro_usa(loc=as.numeric(ptdata[j,c("decimalLongitude","decimalLatitude")]), dstart=startday, dfinish=endday,
                            ...
                            runshade=runshade,runmoist=runmoist,soilgrids=soilgrids,minshade=fpar_pt),
  ...
  dem_pt_list[[j]]$rbs_presence <- ptdata$rbs_presence[j]
  
  write.csv(dem_pt_list[[j]],paste(out_path,"microusa_20y_shade_rbs",j,".csv",sep=""))
}

dem_pt_df <- do.call(rbind,dem_pt_list)
write.csv(dem_pt_df,paste(out_path,"20y_Shade_microclimate_summary_rbs.csv",sep=""))
...
monthly_mean <- aggregate(cbind(maxt,mint)~lat+lon+ptID+rbs_presence+month+year,dem_pt_df,mean)
monthly_precip <- aggregate(prec~lat+lon+ptID+rbs_presence+month+year,dem_pt_df,sum)
...
names(bioclim.vars) <- c(paste("bio",1:19,sep=""),"lat","lon","ptID","rbs_presence","year")
...
for(i in 1:length(pt_by_year)){
  ...
  bioclim.vars[i,]$rbs_presence <- dat$rbs_presence[1]
  bioclim.vars[i,]$year <- dat$year[1]
}

bioclim.vars$year <- NULL
mean_bioclim <- aggregate(.~lat+lon+ptID+rbs_presence,bioclim.vars,mean)

write.csv(mean_bioclim,paste(sp_path,"bioclim_20ymean_shademicroclimate_rbs.csv",sep=""))
